...
import { getAllLife, getLifeBySlug } from "@/lib/life";
...
export async function generateStaticParams() {
  return getAllLife().map((l) => ({ slug: l.slug }));
}

export default async function LifeDetail({ params }: Props) {
  const { slug } = await params;
  const item = getLifeBySlug(slug);

  if (!item) notFound();

  const { older, newer } = getAdjacent("life", slug);
  ...
  return (
    <main className="page-bg relative min-h-screen overflow-hidden bg-[#050505] text-white">
      ...
      <div className="pointer-events-none absolute right-0 top-0 h-[450px] w-[450px] -translate-y-1/3 translate-x-1/4 rounded-full bg-rose-500/[0.06] blur-3xl" />
      <Navbar />
      <TableOfContents />
      <EditInKeystatic section="life" slug={slug} />
